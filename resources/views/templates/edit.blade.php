        ...
        <div class="row">
            <div id="flLoginForm" class="col-lg-12 layout-spacing">
                <div class="statbox widget box box-shadow">
                    ...
                    <br>
                    <form action="{{ route('templates.update', $templates->id) }}" method="POST" enctype="multipart/form-data" class="row g-3" style="margin-left:2rem; margin-right:2rem;">
                        @csrf
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select" name="category" id="category" required>
                                ...
                                @foreach ($templatecats as $templatecat)
                                    <option value="{{ $templatecat->name }}" {{ $templatecat->name == $templates->category ? 'selected' : '' }}>
                                        {{ ucfirst($templatecat->name) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <textarea class="form-control" name="content" placeholder="Enter Content" rows="5">{{ $templates->content }}</textarea>
                        </div>
                        ...
                        <!-- Pre-check checkboxes based on stored skills -->
                        <div class="checkbox-group">
                            @foreach($skills as $skill)
                                <label class="checkbox-container">
                                    <input type="checkbox" class="skill-checkbox" name="skills[]" value="{{ $skill->name }}" 
                                        {{ in_array($skill->name, $selectedSkills) ? 'checked' : '' }}>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            buttons.forEach(button => {
                button.addEventListener('click', function() {
                    const skillsToCheck = this.id.split(', '); 
